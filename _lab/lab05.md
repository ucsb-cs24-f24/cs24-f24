...
assigned: 2024-10-31 01:00:00.00-7
due: 2024-11-13 23:59:00.00-8
---
...
In this lab you will implement a game in two different ways: one that uses the STL `set` container class and another that uses your custom implementation of a BST (based on the code your wrote for [lab02](../lab02)).

### Starter code and required files

Refer to [lab01](../lab01) for instructions on how to set up a GitHub repository and pull the starter code for this lab. Obtain the starter code from this repo:
...
* `card.cpp` and `card.h`: These files should define any structures needed to represent a single card and associated operators. The classes you define here should be useful for both implementations of the game.
...
* `card_list.cpp` and `card_list.h`: these files should contain your implementation of the binary search tree to store a sequence of cards representing a player's hand
* `main.cpp`
...
You and your partner only need to make a single submission. Add your partner to Gradescope before you submit (or before the deadline). Submit your code on Gradescope. You must organize your program in the files: `main_set.cpp`,  `main.cpp`, `card.cpp`, `tests.cpp`, `card.h`, `card_list.cpp`, `card_list.h`, and `Makefile`.
